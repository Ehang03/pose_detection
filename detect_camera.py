...
def workouts(model_path, video_path=None, point_list=[6, 8, 10], up_angle=130, down_angle=90, show_fps=True):
    # 初始化 YOLO 模型
...
if __name__ == '__main__':
    ...
    # 使用摄像头进行实时姿态识别 - 默认使用俯卧撑姿势（右肩、右肘、右手）
    point_list = [6, 8, 10]  # 右肩、右肘、右手
    total_reps = workouts(model_path, point_list=point_list, up_angle=130, down_angle=90)

    # # 使用摄像头进行实时仰卧起坐识别
    # # 关键点说明：6(右肩)、12(右髋)、14(右膝) - 用于检测躯干弯曲角度
    # point_list = [6, 12, 14]  # 右肩、右髋、右膝
    # # 仰卧起坐角度阈值：起身时角度>120°，躺下时角度<70°
    # total_reps = workouts(model_path, point_list=point_list, up_angle=120, down_angle=70)
